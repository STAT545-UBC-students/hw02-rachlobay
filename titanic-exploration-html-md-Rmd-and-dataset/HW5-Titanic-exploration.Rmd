...
Now, I will convert survived to a factor and change it's factor levels to "no" and "yes".

```{r}
survived.factor <- as.factor(survived) # convert survived to a factor and store in survived.factor

levels(survived.factor) # check what the factor levels of survived are.

levels(survived.factor) <- c("no","yes") # change the factor levels to "no" and "yes"

levels(survived.factor) # check the new levels ar as we expect

```

Next, I will add the new column survived.yes.no to the Titanic data frame.

```{r echo=T, results='hide'}
# note I hid the output here because the whole Titanic data set will output when we mutate the column

# add the new column survived.yes.no to Titanic data frame
survived.factor <- as.factor(survived)
titanic %>% 
  mutate(survived.yes.no = survived.factor) # use dplyr mutate function to add the new column

```

We will check the head of the Titanic data set to make sure the new column was created correctly.

```{r}
head(titanic) # check first six rows of Titanic data set
```
